...
void RecordBasedFileManager::getPageMsg(const void *page, PageMsg &pageMsg) {
    memcpy(&pageMsg, (char *) page + PAGE_SIZE - sizeof(pageMsg), sizeof(PageMsg));
}

void RecordBasedFileManager::setPageMsg(const void *page, PageMsg &pageMsg) {
    memcpy((char *) page + PAGE_SIZE - sizeof(pageMsg), &pageMsg, sizeof(PageMsg));
}

void RecordBasedFileManager::getSlotElements(const void *page, int slotCount, SlotElement slots[]) {
    memcpy(slots, (char *) page + sizeof(PageMsg), sizeof(SlotElement) * slotCount);
}

void RecordBasedFileManager::setSlotElements(const void *page, int slotCount, SlotElement slots[]) {
    memcpy((char *) page + sizeof(PageMsg), slots, sizeof(SlotElement) * slotCount);
}
    ...
    if (pageCount > 0) {
        ...
        for (int i = pageCount - 1; i >= 0; i--) { //else insert from page 0
            if (freeList[i] >= recordSize + sizeof(SlotElement)) {
                ...
                PageMsg pageMsg{};
                this->getPageMsg(page,pageMsg);

                SlotElement slots[pageMsg.slotCount + 1];
                this->getSlotElements(page,pageMsg.slotCount,slots);
                ...
                this->setPageMsg(page,pageMsg);

                //maintain slots
                slots[slotNum].offset = offset;
                slots[slotNum].length = recordSize;
                this->setSlotElements(page,pageMsg.slotCount,slots);
